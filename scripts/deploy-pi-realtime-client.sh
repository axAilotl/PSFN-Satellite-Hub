...
VOICE_START_THRESHOLD="${VOICE_START_THRESHOLD:-0.006}"
...
VOICE_INITIAL_SILENCE_TIMEOUT_SECONDS="${VOICE_INITIAL_SILENCE_TIMEOUT_SECONDS:-1.3}"
VOICE_END_SILENCE_TIMEOUT_SECONDS="${VOICE_END_SILENCE_TIMEOUT_SECONDS:-0.6}"
...
spawn ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${user}@${host} bash -lc $remote_cmd
...
printf '%s\n' '${PI_PASSWORD}' | sudo -S systemctl enable --now opanhome-realtime-client.service
